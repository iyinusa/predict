...
class Club extends CI_Controller {
	...
	public function index() {
		...
		if($get_id != ''){
			$gq = $this->mclub->query_club_id($get_id);
			foreach($gq as $item){
				$data['e_id'] = $item->id;
		...
		if($del_id != ''){
			if($this->mclub->delete_club($del_id) > 0){
		...
		if($_POST){
			...
			if(isset($_FILES['up_file']['name'])){
				...
				if (!$result['status']){
					$data['err_msg'] ='<div class="alert alert-info"><h5>Can not upload Badge, try another</h5></div>';
				} else {
			...
			if($save_path=='' && $save_path400=='' && $save_path100==''){
				if($club_id != ''){
					...
					if($this->mclub->update_club($club_id, $upd_data) > 0){
				...
				if($club_id != ''){
					...
					if($this->mclub->reg_insert($reg_data) > 0){
						$data['err_msg'] = '<div class="alert alert-info"><h5>Successfully</h5></div>';
					} else {
						$data['err_msg'] = '<div class="alert alert-info"><h5>There is problem this time. Try later</h5></div>';
					}
		...
		//query uploads
		$data['allup'] = $this->mclub->query_club();
		
		$data['log_username'] = $this->session->userdata('log_username');
	  
	  	$data['title'] = 'Clubs | Prediction';
		$data['page_act'] = 'club';
